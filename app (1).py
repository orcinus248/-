...
def filter_schedules_by_profile(schedules, user_profile):
    ...
    for schedule in schedules:
        ...
        # 学科固有の予定をチェック
        title_lower = schedule["title"].lower()
        ...
        # 専攻固有の予定をチェック（海洋学科の3・4年生のみ）
        if user_profile["specialization"] and any(spec in schedule["title"] for spec in ["制御", "機関"]):
            if user_profile["specialization"] not in schedule["title"]:
                include_schedule = False
